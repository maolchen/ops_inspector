...
func initDefaultRules() error {
	...
	defaultRules := []struct {
		...
		{"k8s_cluster", "PVC使用率", true, false, "持久化存储使用率", `100 * (kubelet_volume_stats_capacity_bytes - kubelet_volume_stats_available_bytes) / kubelet_volume_stats_capacity_bytes`, floatPtr(90), "greater", "%", `{"namespace":"命名空间","persistentvolumeclaim":"PVC名称"}`, 0, 100, true},
